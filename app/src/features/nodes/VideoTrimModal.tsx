...
import {
  ...
  clampValue, clampCropPosition, getDisplayedVideoDimensions,
...
interface VideoTrimModalProps {
  ...
  projectId: string;
  onClose: () => void;
  onTrim: (startTime: number, endTime: number, cropParams?: { x: number; y: number; width: number; height: number }) => Promise<void>;
}

type VideoTab = 'trim' | 'adjustments' | 'crop';

export function VideoTrimModal({ videoUrl, videoNodeId, projectId, onClose, onTrim }: VideoTrimModalProps): React.ReactElement {
  ...
  useEffect(() => {
    ...
    if (!video) return;
    const onMeta = () => {
      setDuration(video.duration); setStartTime(0); setEndTime(video.duration); setCurrentTime(0); video.currentTime = 0;
      const vw = video.videoWidth || 1920, vh = video.videoHeight || 1080;
      setVideoNaturalWidth(vw); setVideoNaturalHeight(vh);
      const iw = Math.round(vw * 0.8), ih = Math.round(vh * 0.8);
      setCropX(Math.round((vw - iw) / 2)); setCropY(Math.round((vh - ih) / 2));
      setCropWidth(iw); setCropHeight(ih); setCropWidthInput(iw.toString()); setCropHeightInput(ih.toString());
    };
  ...
  const handleTimelineClick = useCallback((e: React.MouseEvent<HTMLDivElement>) => {
    ...
    if (Math.abs(time - startTime) < Math.abs(time - endTime)) {
      setStartTime(Math.min(time, endTime - 0.1)); if (videoRef.current) videoRef.current.currentTime = time;
    } else {
      setEndTime(Math.max(time, startTime + 0.1)); if (videoRef.current) videoRef.current.currentTime = time;
    }
  ...
  useEffect(() => {
    ...
    const onMove = (e: MouseEvent) => {
      ...
      const rect = timelineRef.current.getBoundingClientRect();
      const time = Math.max(0, Math.min(1, (e.clientX - rect.left) / rect.width)) * duration;
      if (dragging === 'start') { const ns = Math.max(0, Math.min(time, endTime - 0.1)); setStartTime(ns); if (videoRef.current) videoRef.current.currentTime = ns; }
      else { const ne = Math.min(duration, Math.max(time, startTime + 0.1)); setEndTime(ne); if (videoRef.current) videoRef.current.currentTime = ne; }
    };
  ...
  const togglePlayPause = useCallback(() => {
    ...
    if (isPlaying) { v.pause(); } else { if (v.currentTime < startTime || v.currentTime > endTime) v.currentTime = startTime; v.play(); }
  }, [isPlaying, startTime, endTime]);

  useEffect(() => {
    const v = videoRef.current; if (!v || !isPlaying) return;
    const check = () => { if (v.currentTime >= endTime) { v.pause(); v.currentTime = startTime; } };
    v.addEventListener('timeupdate', check); return () => v.removeEventListener('timeupdate', check);
  }, [isPlaying, startTime, endTime]);
  ...
  const handleCropPresetChange = useCallback((preset: string) => {
    ...
    if (preset === 'original') { setCropWidth(videoNaturalWidth); setCropHeight(videoNaturalHeight); setCropWidthInput(videoNaturalWidth.toString()); setCropHeightInput(videoNaturalHeight.toString()); setCropX(0); setCropY(0); }
    else if (preset !== 'free') {
      ...
      setCropWidth(Math.round(nw)); setCropHeight(Math.round(nh)); setCropWidthInput(Math.round(nw).toString()); setCropHeightInput(Math.round(nh).toString());
      setCropX(Math.round((videoNaturalWidth - nw) / 2)); setCropY(Math.round((videoNaturalHeight - nh) / 2));
    }
  ...
  const handleCropWheel = useCallback((e: React.WheelEvent<HTMLDivElement>) => {
    ...
    let nw = clampValue(cropWidth + delta, 50, videoNaturalWidth), nh = nw / ratio;
    if (nh > videoNaturalHeight) { nh = videoNaturalHeight; nw = nh * ratio; }
    if (nh < 50) { nh = 50; nw = nh * ratio; }
    const clamped = clampCropPosition(cropX - (nw - cropWidth) / 2, cropY - (nh - cropHeight) / 2, nw, nh, videoNaturalWidth, videoNaturalHeight);
    setCropX(Math.round(clamped.x)); setCropY(Math.round(clamped.y)); setCropWidth(Math.round(nw)); setCropHeight(Math.round(nh));
  ...
  const handleTrim = useCallback(async () => {
    ...
    try {
      setIsTrimming(true); setError(null);
      ...
      await onTrim(startTime, endTime, cropParams); onClose();
    } catch (err) { setError(err instanceof Error ? err.message : 'Failed to trim video'); }
    ...
    <div className="fixed inset-0 z-[9999] flex items-center justify-center bg-black/80 backdrop-blur-sm" onClick={onClose}>
      <div className="relative flex w-full max-w-4xl flex-col overflow-hidden rounded-3xl border border-white/15 bg-gradient-to-br from-slate-900/95 to-slate-950/95 shadow-2xl backdrop-blur-xl" onClick={(e) => e.stopPropagation()}>
        <header className="flex items-center justify-between border-b border-white/10 px-6 py-4 sm:px-8">
          <h2 className="text-lg font-semibold text-white">Video Crop</h2>
        ...
        <div className="flex flex-col gap-6 px-6 py-6 sm:px-8">
          ...
          <div className="w-full rounded-2xl border border-white/10 bg-black/25 p-4 text-xs text-white/80">
            <div className="grid gap-2 sm:grid-cols-4">
              <div className="flex flex-col gap-1"><span className="text-white/40">Start</span><span className="text-white/70 font-mono">{formatTime(startTime)}</span></div>
              <div className="flex flex-col gap-1"><span className="text-white/40">Fragment Duration</span><span className="text-white/70 font-mono">{formatTime(endTime - startTime)}</span></div>
              <div className="flex flex-col gap-1"><span className="text-white/40">End</span><span className="text-white/70 font-mono">{formatTime(endTime)}</span></div>
          ...
          <div className="flex flex-wrap items-center justify-center gap-2">
            <button type="button" onClick={() => setActiveTab('trim')} className={tabCls(activeTab === 'trim')}>Time Crop</button>
          ...
          {activeTab === 'trim' && (
            <div className="rounded-2xl border border-white/10 bg-black/30 p-4">
              <div className="mb-2 flex items-center justify-between">
                <span className="text-xs uppercase tracking-wide text-white/40">Select fragment to trim</span>
              ...
              <div ref={timelineRef} className="relative h-10 bg-slate-700 rounded-full cursor-pointer" onClick={handleTimelineClick}>
                <div className="absolute top-0 bottom-0 bg-sky-400/40 rounded-full" style={{ left: `${startPct}%`, right: `${100 - endPct}%` }} />
                <div className="absolute top-1/2 -translate-y-1/2 w-4 h-8 bg-sky-400 rounded border-2 border-white cursor-ew-resize hover:bg-sky-300 transition-colors shadow-lg" style={{ left: `calc(${startPct}% - 8px)` }} onMouseDown={(e) => { e.preventDefault(); e.stopPropagation(); setDragging('start'); }}>
                  <div className="absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 text-white text-xs">{'\u2016'}</div>
                </div>
                <div className="absolute top-1/2 -translate-y-1/2 w-4 h-8 bg-sky-400 rounded border-2 border-white cursor-ew-resize hover:bg-sky-300 transition-colors shadow-lg" style={{ left: `calc(${endPct}% - 8px)` }} onMouseDown={(e) => { e.preventDefault(); e.stopPropagation(); setDragging('end'); }}>
                  <div className="absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 text-white text-xs">{'\u2016'}</div>
                </div>
              </div>
        ...
        <footer className="flex items-center justify-end gap-3 border-t border-white/10 px-6 py-4 sm:px-8">
          <button type="button" onClick={onClose} disabled={isTrimming} className="rounded-lg border border-white/10 bg-white/5 px-4 py-2 text-sm text-white/70 transition hover:bg-white/10 disabled:opacity-50">Cancel</button>
          <button type="button" onClick={handleTrim} disabled={isTrimming || !duration || endTime <= startTime} className="rounded-lg bg-sky-500/80 px-5 py-2 text-sm font-semibold text-white shadow hover:bg-sky-500 disabled:opacity-50">{isTrimming ? 'Processing...' : 'Apply'}</button>
        </footer>
      </div>
    </div>,
    document.body,
